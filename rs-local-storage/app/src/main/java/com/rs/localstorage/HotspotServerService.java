...
public class HotspotServerService extends Service {
    ...
    void handle(Socket s){ try(Socket socket=s){ if(!sameSubnet(socket.getInetAddress()))return; socket.setSoTimeout(15000); BufferedReader br=new BufferedReader(new InputStreamReader(socket.getInputStream(),StandardCharsets.UTF_8)); String req=br.readLine(); if(req==null)return; String[] parts=req.split(" "); if(parts.length<2)return; String method=parts[0], path=URLDecoder.decode(parts[1],"UTF-8"); Map<String,String> h=new HashMap<>(); String line; int clen=0; while((line=br.readLine())!=null&&!line.isEmpty()){int k=line.indexOf(':'); if(k>0)h.put(line.substring(0,k).trim().toLowerCase(),line.substring(k+1).trim());} if(h.containsKey("content-length"))clen=Integer.parseInt(h.get("content-length")); OutputStream out=socket.getOutputStream(); String ip=socket.getInetAddress().getHostAddress();
